...
public class ScheduleUtils {
	...
	public static TriggerKey getTriggerKey(ScheduleJobEntity scheduleJob) {
		if (scheduleJob.getId() == null) {
			//id为null，说明是手动执行的，设置name为类型id+类型名称，避免不同任务不可同时执行
			return TriggerKey.triggerKey(JOB_NAME + scheduleJob.getTypeId() + "_" + scheduleJob.getJobType(), scheduleJob.getJobGroup());
		}
	...
	public static JobKey getJobKey(ScheduleJobEntity scheduleJob) {
		if (scheduleJob.getId() == null) {
			//id为null，说明是手动执行的，设置name为类型id+类型名称，避免不同任务不可同时执行
			return JobKey.jobKey(JOB_NAME + scheduleJob.getTypeId() + "_" + scheduleJob.getJobType(), scheduleJob.getJobGroup());
		}
